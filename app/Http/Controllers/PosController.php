...
use App\Models\SalesDetailTenant;
use App\Models\SalesTenant;
...
class PosController extends BaseController
{
    ...
    public function CreatePOS(Request $request)
    {
        ...
        $item = DB::transaction(function () use ($request) {
            ...
            // create sales tenant schema master
            $tenantSales = SalesTenant::create([
                'is_pos' => 1,
                'date' => Carbon::now(),
                'Ref' => app('App\Http\Controllers\SalesController')->getNumberOrder(),
            ...
            foreach ($data as $key => $value) {
                ...
                $unitName = Unit::where('id', $value['sale_unit_id'])->value('name');

                if (!empty($value['product_variant_id'])) {
                    $variantName = ProductVariant::where('id', $value['product_id'])->value('name');
                } else {
                    $variantName = null;
                }

                $tenantSales->details()->create([
                    'date' => Carbon::now(),
                    'sale_unit_id' => $value['sale_unit_id'],
                    'quantity' => $value['quantity'],
                    'product_id' => $value['product_id'],
