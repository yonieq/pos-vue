...
class SalesController extends BaseController
{
    ...
    public function set_printer(Request $request, $id)
    {
        ...
        $dateSales = $sale->date;
        $nameStore = $settings->CompanyName;
        // $cashier = Auth::user()->firstname . ' ' . Auth::user()->lastname;
        $customerName = $sale->client_name;
        $footerPrint = $pos_settings->note_customer;

        $getKasir = User::where('id', $sale->user_id)->first();

        // dd(Auth::user());


        $resultString = "<111>$nameStore
        <000>Date     : $dateSales
        <000>Cashier  : $getKasir->firstname  $getKasir->lastname
        <000>--------------------------------";

        $totalPrice = 0;

        // Loop produk dan detailnya
        foreach ($sale['details'] as $detail) {
            if ($detail->sale_unit_id !== null) {
                $unit = Unit::where('id', $detail->sale_unit_id)->first();
            } else {
            ...
            if ($detail->product_variant_id) {
                ...
                $productName = $productsVariants->name . '-' . $detail['product']['name'];
            } else {
                $productName = $detail['product']['name'];
            }

            $totalProductPrice = $detail->quantity * $detail->price;
            $totalPrice += $totalProductPrice;

            // Menambahkan baris produk ke string hasil
            $resultString .= "\n<000>$productName";
            // $resultString .= "\n<000>{$detail->quantity} {$unit->ShortName} x {$detail->price}   {$totalProductPrice}";
            $resultString .= "\n<000>" . $detail->quantity . " {$unit->ShortName} x " . number_format($detail->price, 2) . "   " . number_format($totalProductPrice, 2);
        }

        $resultString .= "\n<000>--------------------------------";
        $resultString .= "\n<100>Order Tax         : IDR " . number_format($sale->taxe, 2) . ' (' . $sale->tax_rate . '%)';
        $resultString .= "\n<100>Discount          : IDR " . number_format($sale->discount, 2);
        $resultString .= "\n<100>Grand Total       : IDR " . number_format($sale->GrandTotal, 2);
        $resultString .= "\n<100>Paid By           : " . $payments->first()->Reglement;
        $resultString .= "\n<100>Change Return     : IDR " . number_format($payments->first()->change, 2);
        $resultString .= "\n<000>--------------------------------";
        $resultString .= "\n<000>$footerPrint";
